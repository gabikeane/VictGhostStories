...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.w3.org/2000/svg"
    ...
    <xsl:variable name="xLength" select="1200" as="xs:integer"/>

    <xsl:template match="/">
        <svg height="500" width="1500">
            <g transform="translate(60, 330)">
                <line x1="20" x2="20" y1="0" y2="-250" stroke="black" stroke-width="1"/>
                <line x1="20" x2="{$xLength}" y1="0" y2="0" stroke="black" stroke-width="1"/>
                <line x1="20" x2="{$xLength}" y1="-100" y2="-100" stroke="black" opacity="0.5"
                    stroke-dasharray="8 4" stroke-width="1"/>
                <line x1="20" x2="{$xLength}" y1="-200" y2="-200" stroke="black" opacity="0.5"
                    stroke-dasharray="8 4" stroke-width="1"/>
    ...
    <xsl:template match="story" mode="key">
        ...
        <xsl:variable name="title"
            select="translate(translate(normalize-space(translate(title, '''', '')), ' ', ''), '-', '')"/>
        <g transform="translate(0, 10)">
            <rect x="960" y="{$yPosition}" stroke="black" fill="pink" height="10" width="10"
                class="{$title}"/>
            <text x="975" y="{$yPosition + 10}" stroke="black">
    ...
    <xsl:template match="story" mode="one">
        ...
        <xsl:variable name="title"
            select="translate(translate(normalize-space(translate(title, '''', '')), ' ', ''), '-', '')"/>
        <rect x="{$xPosition}" y="-{$wordcountone}" stroke="black" stroke-width=".5" fill="blue"
            width="{$barWidth}" height="{$wordcountone}" class="{$title}"/>
        <text x="{$xPosition}" y="{-$wordcountone - 2}" text-anchor="inherit">
            <xsl:value-of select="num[@scale = '1']"/>
        </text>
    ...
    <xsl:template match="story" mode="two">
        ...
        <text x="{$xPosition +$barShift}" y="{-$wordcounttwo - 2}" text-anchor="inherit">
            <xsl:value-of select="num[@scale = '2']"/>
        </text>
    ...
    <xsl:template match="story" mode="three">
        ...
        <text x="{$xPosition + 2*$barShift}" y="{-$wordcountthree - 2}" text-anchor="inherit">
            <xsl:value-of select="num[@scale = '3']"/>
        </text>
    ...
    <xsl:template match="story" mode="zero">
        ...
        <text x="{$xPosition + 3*$barShift}" y="{-$wordcountzero - 2}" text-anchor="inherit">
            <xsl:value-of select="num[@scale = '0']"/>
        </text>
        <text x="{100 + 3*$barShift}" y="15" text-anchor="left">0</text>
    </xsl:template>
    ...
    <xsl:template match="story" mode="i">
        ...
        <text x="{$xPosition + 4*$barShift}" y="{-$wordcounti - 2}" text-anchor="inherit">
            <xsl:value-of select="num[@scale = 'i']"/>
        </text>
